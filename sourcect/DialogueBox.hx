...
class DialogueBox extends FlxSpriteGroup
{
	...
	public function new(talkingRight:Bool = true, ?dialogueList:Array<String>)
	{
		...
		else if (PlayState.SONG.song.toLowerCase()=='egoism')
		{
			portraitLeft = new FlxSprite(200, 200);
			portraitLeft.frames = Paths.getSparrowAtlas('portraits/ctportrait' , 'shared');
		...
		{			
			portraitRight = new FlxSprite(600, 200);
		...
		{
		handSelect = new FlxSprite(FlxG.width * 0.9, FlxG.height * 0.9).loadGraphic(Paths.image('portraits/arrow' , 'shared'));
		add(handSelect);
		}
		...
		if (PlayerSettings.player1.controls.ACCEPT && dialogueStarted == true)
		{
			...
			if (dialogueList[1] == null && dialogueList[0] != null)
			{
				if (!isEnding)
				{
					...
					if (PlayState.SONG.song.toLowerCase() == 'senpai' || PlayState.SONG.song.toLowerCase() == 'thorns' || PlayState.SONG.song.toLowerCase() == 'egoism')
